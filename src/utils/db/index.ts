...
const test = async () => {
  await DB.app("test").from("post").id("abc123").get("title").text();
  await DB.app("test").from("post").id("abc123").get("created").numeric();
  await DB.app("test").from("post").id("abc123").get("content").document();
  await DB.app("test").from("post").id("abc123").get("image").file();

  await DB.app("test").from("post").id("abc123").set("title").text("");
  await DB.app("test").from("post").id("abc123").set("created").numeric(0);
  await DB.app("test").from("post").id("abc123").set("content").document("");
  await DB.app("test").from("post").id("abc123").set("image").file([]);

  await DB.app("test").from("post").query().ids();
  await DB.app("test").from("post").query({ skip: 24 }).ids();
  await DB.app("test").from("post").query({ take: 42 }).ids();
  await DB.app("test").from("post").query({ order: "asc" }).ids();
  await DB.app("test").from("post").query({ order: "desc" }).ids();

  await DB.app("test").from("post").query().count();
  await DB.app("test").from("post").query({ skip: 24 }).count();
  await DB.app("test").from("post").query({ take: 42 }).count();
  await DB.app("test").from("post").query({ order: "asc" }).count();
  await DB.app("test").from("post").query({ order: "desc" }).count();

  await DB.app("test").from("post").query().find("title").text("Foo");
  await DB.app("test").from("post").query().find("title").text({ contains: "oo" });

  await DB.app("test").from("post").query().find("created").numeric(123);
  await DB.app("test").from("post").query().find("created").numeric({ gt: 122 });

  await DB.app("test").from("post").query().find("content").document("Foo");
  await DB.app("test").from("post").query().find("content").document({ contains: "oo" });

  await DB.app("test").drop("post");
  await DB.app("test").from("post").drop("abc123");
  await DB.app("test").from("post").id("abc123").drop("title");
};
      ...
      query: (opts?: { skip?: number; take?: number; order?: "asc" | "desc" }) => {
        const Args = async () => {
          ...
          return {
            ...
            skip,
            take,
            where: { AND: { domain_id, NOT: { id: "undefined" } } },
          };
        };

        const count = async () => {
          const args = await Args();
          const count = await orm.entity.count({
            ...args,
            select: { id: true },
          });
          return count.id;
          ...
          return {
            text: async (where: StringFilter | string) => {
              ...
              return await Entities({
                domain: { id: args.where.AND.domain_id },
                attribute: await orm.text.findMany({
                  ...args,
                  select: { attribute_id: true },
                  where: { AND: { value: where, NOT: { id: "undefined" } } },
                }).then((values) => {
